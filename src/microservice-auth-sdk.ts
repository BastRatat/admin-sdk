/**
 * MicroserviceAuthSDK class for server-side microservice authentication
 */
...
import { AuthError, PermissionError, ConfigError } from "./types";
 ...
 */
export class MicroserviceAuthSDK {
  ...
  > {
    try {
      ...
      const { data, error } = await this.supabase.auth.admin.createUser({
        ...
        email_confirm: true, // Auto-confirm for microservice signup
        app_metadata: {
          services: [this.config.serviceName], // Grant access to current service
          roles: {
            [this.config.serviceName]: "user", // Default role
          },
        },
        user_metadata: {
          ...options.userData,
   ...
   */
  middleware(options: MicroserviceMiddlewareOptions = {}) {
    const {
      requireAuth = true,
    ...
    return async (req: any, res: any, next: any) => {
      try {
        ...
        req.user = context;
        req.session = { access_token: token }; // Simplified session object
  ...
  private validateConfig(): void {
    ...
    if (!this.config.supabaseServiceKey.startsWith("eyJ")) {
      throw new ConfigError(
        ...
        "INVALID_SERVICE_KEY",
        "Service role key should be a JWT token",
        "Check the Supabase service role key format"
      );
    }

    // Ensure we're not accidentally using the anon key
    if (this.config.supabaseServiceKey.includes("anon")) {
      throw new ConfigError(
        "Anon key detected",
        "ANON_KEY_USED",
        "Use service role key for server operations, not anon key",
